...
| 角色 | code | 权限 | 说明 |
|------|------|------|------|
| 超级管理员 | `system_admin` | `["*"]` | 系统级，可访问所有租户 |
| 租户管理员 | `admin` | `["*"]` | 租户内全部权限 |
| 运维人员 | `operator` | 按需分配 | 日常运维操作 |
| 只读用户 | `viewer` | `["read"]` | 仅查看 |
...
│   │   │   ├── login.tsx     # 登录
│   │   │   ├── register.tsx  # 注册
│   │   │   └── diagnose.tsx  # Trace 诊断
│   │   ├── components/        # 通用组件
│   │   │   ├── ui/           # shadcn/ui 组件
│   │   │   ├── common/       # 通用业务组件（Modal、Badges 等）
│   │   │   ├── condition/    # 条件选择组件
│   │   │   ├── layout/       # 布局组件
│   │   │   ├── alerts/       # 告警相关组件
│   │   │   ├── rules/        # 规则相关组件
...
└──────────────────────────────┼──────────────────────────────────┘
                    ...
                    └─────────┬─────────┘
                              │
                    ┌─────────▼─────────┐
                    │  PGMQ alerts_raw  │  ← 消息队列缓冲
                    └─────────┬─────────┘
                              │
┌──────────────────────────────▼──────────────────────────────────┐
│                        核心处理层                                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────────┐ │
│  │ AlertDispatcher│ │ 规则引擎  │  │ 去重/聚合 │  │   AI 增强模块   │ │
│  │ (消费者)  │  │          │  │          │  │                  │ │
│  └─────┬────┘  └─────┬─────┘  └────┬─────┘  └────────┬─────────┘ │
│        └────────────┴─────────────┴──────────────────┘          │
└──────────────────────────────┼──────────────────────────────────┘
...
```
告警接入 → PGMQ alerts_raw → AlertDispatcher 消费 → 指纹生成 → 去重检查 → 抑制检查 → 聚合检查 → 规则匹配(AND/OR条件) → PGMQ alerts_notify → NotificationWorker → 多渠道发送
```
...
独立页面 `/alerts/unresolved`，从告警看板"查看全部"或侧边栏"告警看板 → 未恢复告警"进入。支持：

- **筛选**: 严重级别、持续时长、仅抖动、仅长时间未更新
- **排序**: 持续时长、严重级别、告警数量（升序/降序）
- **分页**: 加载更多模式，累积显示已加载数据
...
| 队列 | alerts_escalation（PGMQ） |

**职责**：从 PGMQ `alerts_escalation` 队列消费升级事件，监控触发中（firing）且未确认的告警，按等待时间表自动升级：
...
| 启动方式 | main.py lifespan asyncio.create_task |
| 队列 | alerts_notify（PGMQ）|

**职责**：从 PGMQ `alerts_notify` 队列消费通知消息，调用 ChannelFactory 发送至各渠道（钉钉、飞书、企业微信、邮件、Webhook、Slack）。

处理流程：mq.receive → 查询告警+渠道配置 → 渲染模板 → 发送 → mq.ack → 写入 notification_records。
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `max_fired_at` | ISO datetime | 持续时长筛选：`fired_at <= 该值` |
| `flapping_only` | bool | 仅显示抖动告警 |
| `stale_only` | bool | 仅显示长时间未更新告警 |
| `sort_by` | string | 排序字段：`duration`（持续时长）/ `severity`（严重级别）/ `count`（告警数量） |
...
| namespace_desc | 命名空间中文名 | 云服务器 ECS, 云数据库 RDS |
| product | 产品中文名称 | 阿里云ECS, 阿里云RDS |
| metric_name | 指标名称 | CPUUtilization |
| metric_name_desc | 指标名称中文名 | CPU 利用率 |
