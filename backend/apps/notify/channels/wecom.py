    ...
    async def send(self, alert: Alert, template: str = None) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if result.get("errcode") == 0:
                logger.info("wecom_send_success", alert_id=alert.id)
                return True, None
            else:
                error_msg = result.get("errmsg", "Unknown error")
                logger.error("wecom_send_error", alert_id=alert.id, error=error_msg)
                return False, error_msg

        except Exception as e:
            logger.error("wecom_send_exception", alert_id=alert.id, error=str(e))
            return False, str(e)
